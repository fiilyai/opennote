...
B 站视频的口播不在网页正文里，字幕又要登录才拿得到，所以 `browser` + 网页抽取那套抓不到正文。本 skill 用一个脚本把「B 站链接 → 中文文字」打通：取元信息 + 把音频用语音识别转写。
...
1. **抓取并存原文（raw）**。运行脚本，加 `--save raw` 让它**按视频真实标题自己命名并写盘**，文件名别自己编（尤其别用 BV 号）：

   ```bash
   node <skill>/scripts/fetch-bilibili.mjs "<bilibili 链接>" --save raw --date <YYYY-MM-DD>
   ```

   - 脚本会把「存到哪」打到 stderr（如 `已存 raw/2026-06-02-真实标题.md`），**用那个路径**填后续摘要页的 `raw:` 和索引，别另编文件名。
   - 转写同时打到 stdout，你能在上下文里看到、供下一步用；raw 由脚本写入，**模型不用把上万字转写当参数重新生成**。
   - 脚本纯 Node（view API 取元信息 → WBI 取纯音频 → 硅基 SenseVoice 转写），不依赖 yt-dlp / lux / ffmpeg。
   - **raw 是不可变原文，保持 ASR 原样**（codeex/cloud 等先别动），符合 wiki-ingest「raw 存源、之后不改」。
...
3. **织入知识库**。按 `wiki-ingest` 流程：raw 已在第 1 步存好 → 写摘要页 → 抽概念/实体页并用 `[[wikilink]]` 互连 → 更新索引/流水。本 skill 负责「拿到 B 站文字 + 存 raw」，wiki 织入交给 wiki-ingest。
